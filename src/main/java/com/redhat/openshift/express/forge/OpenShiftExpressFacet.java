...
public class OpenShiftExpressFacet extends BaseFacet {
   ...
   public boolean isInstalled() {
      try {
         ...
         out.print("Uh oh, something went wrong\n");
         e.printStackTrace();
         return false;
      }
   }
   ...
   public boolean install() {
      try {
         ...
         out.println(ShellColor.RED, "***ERRROR*** " + e.getMessage());
         return false;
      } catch (IOException e) {
         out.print("Uh oh, something went wrong");
         e.printStackTrace();
   ...
   private boolean internalInstall() throws OpenshiftException, IOException {
      Properties conf = new Properties();
      try {
         conf.load(new FileReader(EXPRESS_CONF));
         out.println(ShellColor.YELLOW, "***INFO*** Loaded OpenShift configuration from " + EXPRESS_CONF);
      } catch (Exception e) {
         // Swallow
      }
      ...
      out.println(ShellColor.YELLOW, "***INFO*** Waiting for OpenShift to propagate DNS");
      if (!waitForExpress(application.getApplicationUrl(), out)) {
         out.println(ShellColor.RED, "***ERROR*** OpenShift did not propagate DNS properly");
         return false;
      }
      ...
      out.println(ShellColor.GREEN, "***SUCCESS*** Application deployed to " + application.getApplicationUrl());
   ...
   private boolean waitForExpress(String url, ShellPrintWriter out) {
      for (int i = 0; i < 200; i++) {
         try {
            if (i%5 == 0)
               out.println(ShellColor.YELLOW, "Trying to contact " + url + " (attempt " + (i + 1) + " of 200)");
